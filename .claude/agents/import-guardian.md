...
La web no se fía del estado del evento para elegir el GP a mostrar (MotoGP deja el evento en CURRENT hasta bastante después del domingo): [nextGrandPrixRepository.ts](src/services/db/nextGrandPrixRepository.ts) muestra el primer evento cuya carrera de MotoGP no está FINISHED. Por eso importar el estado de las sesiones es lo que hace que la web pase al siguiente GP.
